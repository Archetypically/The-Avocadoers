    ...
    for dirty_csv in args.file:
        ...
        for row in this_csv_reader:
            if "Week" in row:
                ...
                price_key = "Average Price"
            elif "Date" in row:
                this_row_date = row["Date"]
                this_datetime = datetime.strptime(this_row_date, '%Y-%m-%d')
                price_key = "AveragePrice"
            this_row_value = row[price_key]
            clean_row = {
                "YEAR": this_datetime.strftime("%Y").strip(),
                "WEEK IN YEAR": this_datetime.strftime("%U").strip(),
                "PRICE": this_row_value
            }
        ...
        clean_file_name = dirty_csv.name.replace("fixed", "normalized")
